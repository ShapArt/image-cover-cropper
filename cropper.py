...
def compute_text_like_mask(img_cv: np.ndarray, scale_limit: int = 1024) -> np.ndarray:
    ...
    if max(h, w) > scale_limit:
        ...
        small = cv2.resize(
            img_cv, (int(w * scale), int(h * scale)), interpolation=cv2.INTER_AREA
        )
...
PRODUCT_HINTS = re.compile(
    r"(product|pack|packshot|bottle|tube|jar|box|cream|gel|serum|lotion|sunscreen|spf|bioderma|atoderm|sensibio|sebium|cicabio|photoderm|esthederm|etat)",
    re.I,
)
...
def importance_map(img_cv: np.ndarray, preset: Preset) -> np.ndarray:
    ...
    imp = (
        preset.weights.saliency * sal
        + preset.weights.text * txt
        + preset.weights.face * face
    )
...
def choose_crop(
    ...
    scale_min = (
        zoom_guard_scale_min if not spec.allow_letterbox else preset.zoom_guard_desktop
    )
    for scale in np.linspace(1.0, scale_min, 12):
        ...
        for x in xs:
            for y in ys:
                ...
                score = (
                    inside_inner - 1.2 * moat - (2.0 * label_penalty_boost) * in_label
                )
                if score > best_score:
                    best_score = score
                    best = (x, y, cw, ch)
...
def process_one(
    img: Image.Image, src_name: str, spec_desktop: Spec, spec_mobile: Spec, mode: str
):
    img_cv = pil_to_cv(img)
    if mode == "product":
...
def process_folder(
    ...
    out1: str,
    out2: str,
    label1_size: Tuple[int, int],
    label2_size: Tuple[int, int],
    fmt: str,
    quality: int,
    mode: str,
    max_bytes: int,
    ...
    for root, _, files in os.walk(input_dir):
        for fn in files:
            ...
            try:
                ...
                out_img1, out_img2, preset_name = process_one(
                    img, fn, spec1, spec2, mode
                )
...
def main():
    ap = argparse.ArgumentParser(
        description="Batch cropper with auto presets and size limit"
    )
    ...
    ap.add_argument(
        "--format", dest="fmt", default="jpg", choices=["jpg", "jpeg", "png", "webp"]
    )
    ...
    ensure_dir(args.out1)
    ensure_dir(args.out2)
    process_folder(
        args.input_dir,
        args.out1,
        args.out2,
